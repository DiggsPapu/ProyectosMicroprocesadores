...
void* calculateAdjunta(void* arg)
{
    ...
    for (int k = 0 ; k < 3 ; k++ )
    {
        for (int i = 0; i < 3; i++)
        {
            localM->adjTrans[i][k]=localM->adj[k][i];
        }
...
void* calculateInverse(void* args)
{
    ...
    if (globaldet != 0)
    {
        for (int i = 0 ; i < 3; i++ )
        {
            for (int j = 0; j < 3; j++)
            {
                localM->inv[i][j] = (double) ((double)1/(double)globaldet)*((double)localM->adjTrans[i][j]);    
            }
...
int main()
{
    ...
    matrixInput.determinant = *determinant;
    ...
    while (true)
    {
        ...
        switch (option)
        {
            ...
            case 5:
            {
                // IMPRESION MATRIZ ADJUNTA TRANSPUESTA
                printMatrix(matrixInput.adjTrans);
                break;
            }
            case 6:
            {
            ...
            case 7:
            {
                ...
                firstPos = 0;
                firstPos += matrixInput.matriz[0][0];
                // Creacion de hilos principales
                if  (pthread_create(&tDeterminant, NULL, &calculateDeterminant, (void*)&matrixInput)!=0){ //In case creating threads fail.
                    perror("Failed to create the thread\n");
                    return 1;
